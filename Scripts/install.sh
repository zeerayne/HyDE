...
EOF
	...
	if [ "${flg_DryRun}" -eq 1 ]; then
		# Reports what the real run below would replace; changes nothing.
		...
		# Puts HyDE's template in place of Hyprland's generated example before
		# the deploy, which then adopts it (preserve action) (#2132). The swap
		# is atomic, so an interrupted or failed deploy never leaves the user
		# without a hyprland.lua. A failure only warns: the rest of the restore
