...
conf.XDG_RUNTIME_DIR = os.getenv('XDG_RUNTIME_DIR') or '/run/user/' .. tostring(tonumber(os.getenv('UID')) or 0)
        ...
        crit_source = GLib.timeout_add_seconds(GLib.PRIORITY_DEFAULT, 1, function()
            ...
            notify_send('Battery Critically Low',
                string.format('%d%% is critically low. Device will execute %s in %d:%02d.', pct, conf.execute_critical,
                    mm, ss), { urgency = 'critical', icon = 'battery-empty-symbolic', synchronous = 'hyde.battery' })
...
local function handle_update()
    ...
    local unplug_cond = percentage >= conf.unplug_charger_threshold
    and not string.find(tostring(status), 'Discharging')
    and status ~= 'Full'
    and (percentage - last_notified_percentage) >= conf.interval
if unplug_cond then
    ...
    local low_cond = percentage <= conf.battery_low_threshold
    and starts_with(tostring(status), 'Discharging')
    and (last_notified_percentage - percentage) >= conf.interval
if low_cond then
        local steps = math.floor(((percentage + 5) / 10) + 0.00001) * 10
        ...
        notify_send('Battery Low', string.format('Battery is at %d%%. Connect the charger.', percentage),
            { urgency = 'critical', icon = icon, synchronous = 'hyde.battery' })
        last_notified_percentage = percentage
end
...
local function main(argv)
    for i = 1, #argv do
        ...
        if a == '-v' or a == '--verbose' then log.level = 'debug' end
