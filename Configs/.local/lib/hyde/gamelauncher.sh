...
case "$backend" in
    steam)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/hyde/hyde_pyenv/bin/python" "$LIB_DIR/hyde/gamelauncher/steam.py" --rofi-string)
        ;;
    lutris)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/hyde/hyde_pyenv/bin/python" "$LIB_DIR/hyde/gamelauncher/lutris.py" --rofi-string)
        ;;
    *)
        backend_command=("${XDG_STATE_HOME:-$HOME/.local/state}/hyde/hyde_pyenv/bin/python" "$LIB_DIR/hyde/gamelauncher/catalog.py" --rofi-string)
